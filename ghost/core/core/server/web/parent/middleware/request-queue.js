// as express-queue is a relatively simple package, we can just copy the whole file here
//  to modify for our needs; we may want to fork it later on in order to expose handlers
...
const queueMw = function (config) {
    ...
    self.queueMw = function (req,res,next) {
        ...
        res.once('end', function () {
            job.log('resOnEnd'); 
        }); // Sent on res.end() by express-end
        res.once('finish', function () {
            res.responseTime = (Date.now() - req.startTime) + 'ms';
    ...
    const resultMw = function (req, res, next) {
        endMw(req, res, function () { // Inject res.end() handler to emit 'end' event
            // Appending startTime here ensures our request logging has the start time of the request entering the queue, not the time it left it
            req.startTime = Date.now();
            req.queueDepth = self.jobQueue.getLength(); // Don't add one for this request; a queue depth of 0 makes more sense when the request isn't going to sit in the queue
